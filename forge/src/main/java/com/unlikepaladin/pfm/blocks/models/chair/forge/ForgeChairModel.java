package com.unlikepaladin.pfm.blocks.models.chair.forge;
...
public class ForgeChairModel extends PFMForgeBakedModel {
    public ForgeChairModel(ModelState settings, ModelRenderProperties modelSettings, List<BlockModelPart> templateBakedModels) {
    ...
    public @NotNull ModelData getModelData(@NotNull BlockAndTintGetter world, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull ModelData tileData) {
        if (state.getBlock() instanceof BasicChairBlock) {
            ...
            data = data.derive().with(TUCKED, state.getValue(BasicChairBlock.TUCKED)).build();
            return data;
        }
        return tileData;
    }
