...
public class ForgeChairClassicModel extends PFMForgeBakedModel {
    public ForgeChairClassicModel(ModelState settings, ModelRenderProperties modelSettings, List<BlockModelPart> templateBakedModels) {
    ...
    public @NotNull ModelData getModelData(@NotNull BlockAndTintGetter world, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull ModelData tileData) {
        if (state.getBlock() instanceof ClassicChairBlock) {
            ...
            data = data.derive().with(TUCKED, state.getValue(ClassicChairBlock.TUCKED)).build();
            return data;
        }
        return tileData;
    }
