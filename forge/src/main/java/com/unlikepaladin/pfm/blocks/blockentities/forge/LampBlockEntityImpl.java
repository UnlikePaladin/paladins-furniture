...
public class LampBlockEntityImpl extends LampBlockEntity {
    ...
    public void onDataPacket(Connection net, ClientboundBlockEntityDataPacket pkt, HolderLookup.Provider lookup) {
        ...
        this.color = DyeColor.byName(pkt.getTag().getString("color").orElse("white"), DyeColor.WHITE);
        this.variant = WoodVariantRegistry.getVariant(ResourceLocation.tryParse(pkt.getTag().getString("variant").orElse("minecraft:oak")));
    }
