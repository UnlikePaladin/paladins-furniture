...
public class ForgeKitchenDrawerModel extends PFMForgeBakedModel {
    public ForgeKitchenDrawerModel(ModelBakeSettings settings, ModelSettings modelSettings, List<BlockModelPart> modelParts) {
            ...
            if (block.canConnectToCounter(neighborStateFacing) && neighborStateFacing.contains(Properties.HORIZONTAL_FACING)) {
                ...
                if (direction2.getAxis() != state.get(Properties.HORIZONTAL_FACING).getAxis() && isNeighborStateFacingDifferentDirection) {
                    if (direction2 == direction.rotateYCounterclockwise()) {
                        dest.add(getQuadsWithTexture(getTemplateBakedModels().get((5 + openOffset)), ModelHelper.getOakPlankLogSprites(), spriteList));
                    }
                    else {
                        dest.add(getQuadsWithTexture(getTemplateBakedModels().get((6 + openOffset)), ModelHelper.getOakPlankLogSprites(), spriteList));
                    }
                } else {
                    dest.add(getQuadsWithTexture(getMiddleQuads(left, right, openOffset), ModelHelper.getOakPlankLogSprites(), spriteList));
                }
            }
