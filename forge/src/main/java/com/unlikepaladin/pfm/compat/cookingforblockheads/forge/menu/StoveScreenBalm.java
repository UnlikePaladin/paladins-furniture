...
public class StoveScreenBalm extends AbstractContainerScreen<StoveScreenHandlerBalm> {
    ...
    protected void renderBg(GuiGraphics drawContext, float partialTicks, int mouseX, int mouseY) {
        ...
        drawContext.blit(RenderPipelines.GUI_TEXTURED, texture, this.leftPos + 22 + 61 + offsetX, this.topPos + 18, 176.0F, 117.0F, 76, 76, 256, 256);
        drawContext.blit(RenderPipelines.GUI_TEXTURED, texture, this.leftPos + 22 + 38 + offsetX, this.topPos + 43, 205.0F, 84.0F, 18, 33, 256, 256);
        if (tileEntity.isBurning()) {
            int burnTime = (int)(12.0F * tileEntity.getBurnTimeProgress());
            drawContext.blit(RenderPipelines.GUI_TEXTURED, texture, this.leftPos + 22 + 40 + offsetX, this.topPos + 43 + 12 - burnTime, 176.0F, (float)(12 - burnTime), 14, burnTime + 1, 256, 256);
        }

        if (tileEntity.hasPowerUpgrade()) {
            drawContext.blit(RenderPipelines.GUI_TEXTURED, texture, this.leftPos + this.imageWidth - 25, this.topPos + 22, 205.0F, 0.0F, 18, 72, 256, 256);
            EnergyStorage energyStorage = tileEntity.getEnergyStorage();
            float energyPercentage = (float)energyStorage.getEnergy() / (float)energyStorage.getCapacity();
            drawContext.blit(RenderPipelines.GUI_TEXTURED, texture, this.leftPos + this.imageWidth - 25 + 1, this.topPos + 22 + 1 + 70 - (int)(energyPercentage * 70.0F), 223.0F, 0.0F, 16, (int)(energyPercentage * 70.0F), 256, 256);
        }
