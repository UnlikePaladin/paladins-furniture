...
public abstract class PFMFabricBakedModel extends AbstractBakedModel implements FabricBakedModel, PFMBakedModelParticleExtension {
    ...
    public void pushTextureTransform(RenderContext context, Sprite sprite) {
        context.pushTransform(quad -> {
            Sprite originalSprite = SpriteFinder.get(MinecraftClient.getInstance().getBakedModelManager().method_24153(PlayerScreenHandler.BLOCK_ATLAS_TEXTURE)).find(quad, 0);
            if (originalSprite.getId() != sprite.getId()) {
                for (int index = 0; index < 4; index++) {
                    float frameU = ModelHelper.method_35804(originalSprite, quad.spriteU(index, 0));
                    float frameV = ModelHelper.method_35805(originalSprite, quad.spriteV(index, 0));
    ...
    public void pushTextureTransform(RenderContext context, List<Sprite> toReplace, List<Sprite> replacement, Identifier atlasId) {
        context.pushTransform(quad -> {
            if (replacement != null && toReplace != null ){
                Sprite originalSprite = SpriteFinder.get(MinecraftClient.getInstance().getBakedModelManager().method_24153(atlasId)).find(quad, 0);
