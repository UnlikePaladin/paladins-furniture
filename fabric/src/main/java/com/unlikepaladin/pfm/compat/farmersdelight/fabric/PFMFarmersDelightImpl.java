...
public class PFMFarmersDelightImpl extends PFMFarmersDelight {
    ...
    public void generateTags() {
        List<Block> stoves = new ArrayList<>(StoveBlock.streamStoves().map(FurnitureBlock::getBlock).collect(Collectors.toList()));
        stoves.addAll(IronStoveBlock.streamIronStoves().map(FurnitureBlock::getBlock).collect(Collectors.toList()));
        stoves.add(PaladinFurnitureModBlocksItems.KITCHEN_STOVETOP);
        PFMTagProvider.getOrCreateTagBuilder(Tags.HEAT_SOURCES)
