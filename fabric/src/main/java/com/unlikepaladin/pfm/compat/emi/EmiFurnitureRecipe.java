...
public class EmiFurnitureRecipe extends EmiPatternCraftingRecipe {
    ...
    public EmiFurnitureRecipe(FurnitureRecipe recipe) {
        ...
        for (int i = 0; i < input.size(); i++) {
            ...
            for (EmiStack stack : stacks) {
                ...
                ItemStack remainder = recipe.getRemainingItems(playerInventory).get(i);
    ...
    public SlotWidget getOutputWidget(int x, int y) {
        return new GeneratedSlotWidget((r -> {
            int selectedRecipe = r.nextInt(recipe.getInnerRecipes().size());
            return EmiIngredient.of(Ingredient.of(recipe.getInnerRecipes().get(selectedRecipe).getResultItem()), recipe.getOutputCount());
        }), unique, x, y);
    }
}