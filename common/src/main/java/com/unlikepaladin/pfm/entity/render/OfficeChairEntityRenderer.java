...
public class OfficeChairEntityRenderer extends MobRenderer<OfficeChairEntity, OfficeChairModelEmpty> {
    ...
    public static void renderItem(ItemStack itemStack, PoseStack stack, ItemTransforms.TransformType mode, MultiBufferSource provider,
    ...
    public void render(OfficeChairEntity mobEntity, float f, float g, PoseStack matrixStack, MultiBufferSource vertexConsumerProvider, int light) {
        ...
        List<BakedQuad> baseQuads = Arrays.stream(Direction.values()).map(direction -> modelBase.getQuads(null, direction, mobEntity.level.random))
                .flatMap(List::stream).collect(Collectors.toList());
        ...
        List<BakedQuad> wheelQuads = Arrays.stream(Direction.values()).map(direction -> wheelModel.getQuads(null, direction, mobEntity.level.random))
                .flatMap(List::stream).collect(Collectors.toList());
        ...
        for (float[] wheelOffset : WHEEL_OFFSETS) {
            ...
            matrixStack.mulPose(Vector3f.XP.rotationDegrees(wheelSpin));
            matrixStack.translate(-0.5, -0.08, -0.5);
        ...
        List<BakedQuad> chairQuads = Arrays.stream(Direction.values()).map(direction -> model.getQuads(null, direction, mobEntity.level.random))
                .flatMap(List::stream).collect(Collectors.toList());
