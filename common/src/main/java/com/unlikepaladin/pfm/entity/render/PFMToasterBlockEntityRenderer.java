...
public class PFMToasterBlockEntityRenderer <T extends PFMToasterBlockEntity> extends BlockEntityRenderer<T> {
        ...
        public void render(T blockEntity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
            if (blockEntity instanceof PFMToasterBlockEntity) {
                ...
                matrices.mulPose(Vector3f.YP.rotationDegrees((float)rot));
                matrices.scale(0.8f,0.8f,0.8f);
                matrices.translate(0.0D, 0.0D, -0.55D);
                matrices.translate(0.0D, 0.0D, 0.41D);
                Minecraft.getInstance().getItemRenderer().renderStatic(items.get(0), ItemTransforms.TransformType.GROUND, light, overlay, matrices, vertexConsumers);
                matrices.translate(0.0D, 0.0D, 0.29D);
                Minecraft.getInstance().getItemRenderer().renderStatic(items.get(1), ItemTransforms.TransformType.GROUND, light, overlay, matrices, vertexConsumers);
                matrices.popPose();
            }
        }
