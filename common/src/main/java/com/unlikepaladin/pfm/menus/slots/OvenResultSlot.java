...
public class OvenResultSlot<T extends OvenBlockEntity> extends Slot {
    ...
    protected void checkTakeAchievements(ItemStack stack) {
        ...
        this.removeCount = 0;
        if (this.container instanceof OvenBlockEntity&& ((OvenBlockEntity)container).getLevel() != null && !stack.isEmpty()) {
            if (this.player instanceof ServerPlayer) {
                ((OvenBlockEntity)container).awardUsedRecipesAndPopExperience((ServerPlayer)this.player);
            }
        }
