...
public class UnbakedLogStoolModel implements UnbakedModel {
    public static final ResourceLocation[] LOG_STOOL_PARTS_BASE = new ResourceLocation[] {
            new ResourceLocation(PaladinFurnitureMod.MOD_ID, "block/log_stool/log_stool"),
            new ResourceLocation(PaladinFurnitureMod.MOD_ID, "block/log_stool/log_stool_tucked")
    };
    ...
    public static final List<ResourceLocation> LOG_STOOL_MODEL_IDS = new ArrayList<Identifier>() {
        {
            for(WoodVariant variant : WoodVariantRegistry.getVariants()){
                ...
                add(new ResourceLocation(PaladinFurnitureMod.MOD_ID, "item/" + variant.getSerializedName() + "_" + logType + "_stool"));
                if (variant.hasStripped())
                    add(new ResourceLocation(PaladinFurnitureMod.MOD_ID, "item/stripped_" + variant.getSerializedName() + "_" + logType + "_stool"));
            }
            for(StoneVariant variant : StoneVariantRegistry.getVariants()){
                add(new ResourceLocation(PaladinFurnitureMod.MOD_ID, "item/" + variant.getSerializedName() + "_log_stool"));
            }
            add(STOOL_MODEL_ID);
        }
    };
