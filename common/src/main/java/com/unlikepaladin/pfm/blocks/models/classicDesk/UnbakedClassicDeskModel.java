...
public record UnbakedClassicDeskModel(ModelVariant variant) implements PFMUnbakedBlockStateModel {
    public static final MapCodec<UnbakedClassicDeskModel> MAP_CODEC = RecordCodecBuilder.mapCodec
            (instance ->
                    instance.group(ModelVariant.MAP_CODEC.forGetter(UnbakedClassicDeskModel::variant))
    ...
    public BlockStateModel bake(Baker loader) {
        ...
        for (Identifier modelPart : BASIC_MODEL_PARTS_BASE) {
            bakedModelList.add(GeometryBakedModel.create(loader, modelPart, settings));
        }
