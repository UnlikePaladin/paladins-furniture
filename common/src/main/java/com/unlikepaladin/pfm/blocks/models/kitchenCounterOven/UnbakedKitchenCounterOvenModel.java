...
public class UnbakedKitchenCounterOvenModel implements UnbakedModel {
    ...
    public static final List<String> OVEN_MODEL_PARTS_BASE = new ArrayList<String>() {
        {
            add("block/kitchen_counter_oven/template_kitchen_counter_oven/template_kitchen_counter_oven");
            add("block/kitchen_counter_oven/template_kitchen_counter_oven/template_kitchen_counter_oven_middle");
            add("block/kitchen_counter_oven/template_kitchen_counter_oven/template_kitchen_counter_oven_open");
            add("block/kitchen_counter_oven/template_kitchen_counter_oven/template_kitchen_counter_oven_middle_open");
        }
    };

    private static final Identifier PARENT = new Identifier("block/block");

    public static final List<Identifier> OVEN_MODEL_IDS  = new ArrayList<Identifier>() {
        {
            for(WoodVariant variant : WoodVariantRegistry.getVariants()){
                add(new Identifier(PaladinFurnitureMod.MOD_ID, "block/kitchen_counter_oven/" + variant.asString() + "_kitchen_counter_oven"));
            }
            for(WoodVariant variant : WoodVariantRegistry.getVariants()){
                add(new Identifier(PaladinFurnitureMod.MOD_ID, "block/kitchen_counter_oven/stripped_" + variant.asString() + "_kitchen_counter_oven"));
            }
    ...
    public static final List<Identifier> ALL_MODEL_IDS = new ArrayList<Identifier>() {
        {
            for(WoodVariant variant : WoodVariantRegistry.getVariants()){
                for (String part : OVEN_MODEL_PARTS_BASE) {
    ...
    public UnbakedKitchenCounterOvenModel(VariantBase variant, List<String> modelParts, BlockType type) {
        this.frameTex = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, variant.getTexture(type));
        for(String modelPartName : OVEN_MODEL_PARTS_BASE){
