    ...
    public void run() throws IOException {
        if (!FROZEN) {
            ...
            for (PackResources pack : PFMRuntimeResources.RESOURCE_PACK_LIST) {
                log("\tPack {} {}", pack.location().title().getString().replace("Pack Fabric Mod ", "").replaceAll("\"", ""));
            ...
            PFMCache current = new PFMCache(SharedConstants.getCurrentVersion().name(), Version.getCurrentVersion(), PFMFileUtil.getModLoader(), hashToCompare, variants);
