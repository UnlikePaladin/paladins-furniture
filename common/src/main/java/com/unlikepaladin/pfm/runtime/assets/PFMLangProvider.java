...
public class PFMLangProvider extends PFMProvider {
    ...
    public void generate(String code) throws IOException {
        ...
        try(BufferedWriter writer = IOUtils.buffer(new FileWriter(new File(PFMRuntimeResources.createDirIfNeeded(getParent().getOrCreateSubDirectory("assets/pfm").resolve("lang")).toFile(), code+".json"))))  {
            writer.write("{\n");
    ...
    private Map<String, LanguageInfo> loadAvailableLanguages(Stream<PackResources> packs) {
        ...
        packs.forEach(pack -> {
            try {
                ...
                getParent().getLogger().warn("Unable to parse language metadata section of resourcepack: {}", pack.location().title().toString(), exception);
            }
        });
    ...
    public void loadLanguages(ResourceManager manager) {
        ...
        LanguageInfo enUSDefinition = defs.getOrDefault("en_us", PFMLanguageManagerAccessor.getEnglish_Us());
