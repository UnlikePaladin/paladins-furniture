...
public class NewFurnitureRecipe implements Recipe<PlayerInventory>, Comparable<NewFurnitureRecipe> {
    private final Identifier id;
    final String group;
    final ItemStack output;
    final DefaultedList<Ingredient> input;

    public NewFurnitureRecipe(Identifier id, String group, ItemStack output, DefaultedList<Ingredient> input) {
        this.id = id;
        this.group = group;
        this.output = output;
        this.input = input;
    ...
    public RecipeSerializer<?> getSerializer() {
        return RecipeTypes.NEW_FURNITURE_SERIALIZER;
    }

    @Override
    public RecipeType<?> getType() {
        return RecipeTypes.NEW_FURNITURE_RECIPE;
    }

    @Override
    public int compareTo(@NotNull NewFurnitureRecipe furnitureRecipe) {
        return this.output.toString().compareTo(furnitureRecipe.output.toString());
    }

    public static class Serializer
            implements RecipeSerializer<NewFurnitureRecipe> {
        @Override
        public NewFurnitureRecipe read(Identifier identifier, JsonObject jsonObject) {
            String string = JsonHelper.getString(jsonObject, "group", "");
            DefaultedList<Ingredient> defaultedList = NewFurnitureRecipe.Serializer.getIngredients(JsonHelper.getArray(jsonObject, "ingredients"));
            if (defaultedList.isEmpty()) {
                throw new JsonParseException("No ingredients for furniture recipe");
            }
            ItemStack itemStack = ShapedRecipe.outputFromJson(JsonHelper.getObject(jsonObject, "result"));
            return new NewFurnitureRecipe(identifier, string, itemStack, defaultedList);
        }
        ...
        @Override
        public NewFurnitureRecipe read(Identifier identifier, PacketByteBuf packetByteBuf) {
        ...
        @Override
        public void write(PacketByteBuf packetByteBuf, NewFurnitureRecipe shapelessRecipe) {
