...
public class FreezingWidget extends BurningFire {
    ...
    public void renderBackground(DrawContext context, boolean dark, float alpha) {
        if (getAnimationDuration() > 0) {
            ...
            context.drawTexture(RenderPipelines.GUI_TEXTURED, background, getX(), getY() +2, 56, 36,14, 14, 256, 256);
            context.drawTexture(RenderPipelines.GUI_TEXTURED, background, getX(), getY() + 14 - height, 176, 12 - height, 14, height, 256, 256);

        } else {
            context.drawTexture(RenderPipelines.GUI_TEXTURED, background, getX(), getY(), 1, 74, 14, 14, 256, 256);
        }
    }
