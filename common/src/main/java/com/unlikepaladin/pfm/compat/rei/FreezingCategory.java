...
public class FreezingCategory implements DisplayCategory<FreezingDisplay> {
    ...
    public List<Widget> setupDisplay(FreezingDisplay display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createLabel(new Point(bounds.x + bounds.width - 5, bounds.y + 5), Component.translatable("category.rei.cooking.time&xp", new Object[]{df.format((double)display.getXp()), df.format(cookingTime / 20.0D)})).noShadow().rightAligned().color(-12566464, -4473925));
