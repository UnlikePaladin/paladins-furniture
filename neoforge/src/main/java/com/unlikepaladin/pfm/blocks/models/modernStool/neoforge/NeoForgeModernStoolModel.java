...
public class NeoForgeModernStoolModel extends PFMNeoForgeBakedModel {
    public NeoForgeModernStoolModel(ModelBakeSettings settings, ModelSettings modelSettings, List<BlockModelPart> templateBakedModels) {
    ...
    public void collectParts(BlockRenderView level, BlockPos pos, BlockState state, Random random, List<BlockModelPart> parts) {
        ...
        if (state == null || !(state.getBlock() instanceof ModernStoolBlock))
            return;

        int tucked = state.get(ModernStoolBlock.TUCKED) ? 1 : 0;
