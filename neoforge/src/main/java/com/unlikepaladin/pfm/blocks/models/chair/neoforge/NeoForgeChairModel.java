...
public class NeoForgeChairModel extends PFMNeoForgeBakedModel {
    public NeoForgeChairModel(ModelState settings, ModelRenderProperties modelSettings, List<BlockModelPart> templateBakedModels) {
        super(settings, modelSettings, templateBakedModels);
    }

    @Override
    public void collectParts(BlockAndTintGetter level, BlockPos pos, BlockState state, RandomSource random, List<BlockModelPart> parts) {
        if (state == null || !(state.getBlock() instanceof BasicChairBlock))
            return;

        int tucked = state.getValue(BasicChairBlock.TUCKED) ? 1 : 0;
