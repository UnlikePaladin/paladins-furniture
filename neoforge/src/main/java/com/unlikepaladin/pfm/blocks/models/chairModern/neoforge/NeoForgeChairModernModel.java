...
public class NeoForgeChairModernModel extends PFMNeoForgeBakedModel {
    public NeoForgeChairModernModel(ModelBakeSettings settings, List<BakedModel> templateBakedModels) {
    ...
    public @NotNull ModelData getModelData(@NotNull BlockRenderView world, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull ModelData tileData) {
        if (state.getBlock() instanceof ModernChairBlock) {
            ...
            data = data.derive().with(TUCKED, state.get(ModernChairBlock.TUCKED)).build();
            return data;
        }
        return tileData;
    }
