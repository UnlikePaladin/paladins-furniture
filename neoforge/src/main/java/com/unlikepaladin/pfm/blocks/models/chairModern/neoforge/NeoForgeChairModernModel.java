...
public class NeoForgeChairModernModel extends PFMNeoForgeBakedModel {
    public NeoForgeChairModernModel(ModelState settings, List<BakedModel> templateBakedModels) {
    ...
    public @NotNull ModelData getModelData(@NotNull BlockAndTintGetter world, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull ModelData tileData) {
        if (state.getBlock() instanceof ModernChairBlock) {
            ...
            data = data.derive().with(TUCKED, state.getValue(ModernChairBlock.TUCKED)).build();
            return data;
        }
        return tileData;
    }
